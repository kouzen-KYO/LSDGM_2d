 ...
 * MainIso.cpp
 *
 *  This file is for isotropic compression
 *
 */
...
int main() {
	...
	omp_set_num_threads(6); // Number of threads for parallel
	// Read grain type from input file
	string filename("caicos001.dat");
	vector<Grain2d> grains = generateGrainsFromFile(filename); // Grain container, this function is in 'readInputFile.h'
	vector<Grain2d> temp = grains;
	// Copy already read grain information to finally decide the grain number
	for (size_t i = 0; i < 49; i++) {
		grains.insert(grains.end(), temp.begin(), temp.end());
	}
	size_t ngrains = grains.size();
	cout <<"Number of grains: " << ngrains << endl; // Print the actual grain number to the screen
	// Other grain properties
	double mu = 0.0; // Inter-grain friction
	double kn = 30000; // Normal stiffness
	double density = 1; // Grain density
	// Assembly properties
	double height = 400; // Initial assembly height
	double width = 1100; // Initial assembly width
	// World properties
	double gDamping = .4; // .4 or .2
	double dt = 0.1*(2*sqrt(.5*200/kn)); // Also depend on surface points number
	Vector2d trans(545.07320305-634.37729014-5.5, 784.81266662-656.22997566+5.5);
	Vector2d trans2(0, height/2);

	// Locate grains to the pre-allocate places with initial rotations, the size of these files should be larger than 'ngrains'
	vector<Vector2d> pos = readPositionFile("positions_caicos001.dat", ngrains);
	pos.insert(pos.end(), pos.begin(), pos.end());
	vector<double> rot = readRotationFile("rotations_caicos001.dat", ngrains);
	...
	// Periodic walls in right and left
	...
	// Create output files to save required data
	//	FILE * props;
	FILE * positions;
	FILE * rotations;
	FILE * wallPos;
	...
	vector<Grain2d> grainList; // Grain contain
	double duration, energy;
	double start = omp_get_wtime();

	int tiso = 350000; // Time steps for isotropic compression
	int tshear = 1;	// Time steps for simple shear
	double pressure = 50; // Constant pressure
	double v = .137/2.; // Wall speed

	// Time integration for isotropic compression
	//	size_t iter = 0;
	cout << "Starting iso stage" << endl;
	World2dBiaxial world(grains, walls, wallp, gDamping, dt); // Create world state
