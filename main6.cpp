...
//	size_t iter = 0;
	...
	for (size_t i = 0; i < 3; i++) {
		// expand+rotate grains
		height = world.getWalls()[1].getPosition()(1);
		width = world.getWalls()[3].getPosition()(0);
		double fac = 0.1;
		size_t thetamax = 20;
		if (i == 2)
			thetamax = 30;
		for (size_t i = 0; i < ngrains; i++) {
			world.getGrainsNC()[i].moveGrain( world.getGrains()[i].getPosition()*fac );
//			int randn = rand()%(thetamax*2) - thetamax;
//			world.getGrainsNC()[i].changeRot( world.getGrains()[i].getTheta() + double(randn)*M_PI/180. );
			int randn = rand()%2;
			double sign;
			if (randn == 0) {
				sign = -1;
			...
			world.getGrainsNC()[i].changeRot( world.getGrains()[i].getTheta() + sign*double(thetamax)*M_PI/180. );
		}
		world.getWallsNC()[1].moveWall(Vector2d(0,height*fac));
		world.getWallsNC()[3].moveWall(Vector2d(width*fac,0));
		
		// create world
//		world = World2dBiaxial(world.getGrains(), walls, gDamping, dt);
//		grains.clear();
		...
		for (int t = 0; t < tiso; t++) {
			...
			Vector2d topMove = world.getWallsNC()[1].takeForceTimestep2(inForce-outForce, ws.wallContacts[1]);
			
			inForce = pressure*world.getWalls()[1].getPosition()(1);
			outForce = ws.wallForces[3](0);
			Vector2d rightMove = world.getWallsNC()[3].takeForceTimestep2(inForce-outForce, ws.wallContacts[3]);
			
			Vector2d totMove = topMove + rightMove;
			
			height = world.getWalls()[1].getPosition()(1);
			width = world.getWalls()[3].getPosition()(0);
//			for (size_t i = 0; i < ngrains; i++) {
//				world.getGrainsNC()[i].moveGrain( Vector2d(totMove(0)*world.getGrains()[i].getPosition()(0)/width, totMove(1)*world.getGrains()[i].getPosition()(1)/height ) );
//			}
			world.takeTimestep();
		}
	}
