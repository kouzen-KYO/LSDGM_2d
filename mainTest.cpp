...
int main(int argc, char*argv[]) {
	...
	for (size_t i = 0; i < 50; i++) {
		cout << grains[0].getPointList()[i](0) << " " << grains[0].getPointList()[i](1) << endl;
	}
...
//	mkdir(dir.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
	...
	// assembly properties
	vector<Vector2d> wallp = readPositionFile("wallPos_iso.dat",1);
	double widthInit = wallp[0](1); 
	double heightInit = wallp[0](0);

	
	// world properties
	double gDamping = .3; // .4 or .2
	double dt = 0.2*(2*sqrt(.5*200/kn));
	
	vector<Vector2d> pos = readPositionFile("positions_iso.dat",ngrains);
	vector<double> rot = readRotationFile("rotations_iso.dat",ngrains);
...
//	props 	 = fopen("props.dat","w");
	positions = fopen("positions_test.dat","w");
	rotations = fopen("rotations_test.dat","w");
	wallPos   = fopen("wallPos_test.dat","w");
	stress    = fopen("stress_test.dat","w");
...
//		world.takeTimestep();
	...
	string fname = cinfo + string("/") + to_string(0) + string("_test.dat");
	FILE * cinfofile;
